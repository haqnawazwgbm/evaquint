...
require_once __DIR__.'/TVimageManipulation.php';
class PictureCut{
	...
	private function populateFromArray($data){
        ...
        $last = basename($_SERVER["DOCUMENT_ROOT"]);
        if ($last == 'public') {
            $url = $_SERVER["DOCUMENT_ROOT"];
        } else {
            $url = $_SERVER["DOCUMENT_ROOT"].'/public';
        }
		$this->folderPath = $url . $this->folderOnServer;
	...
	private function populateFileFromStream($fileStream){
		...
		if($this->imageNameRandom == "true"){
			...
			$newName = dechex(round(rand(0,999999999))).".".$this->fileType;
